...
script with nothing but the R code here](../scripts/R-04-visualization-ggplot2.R).
...
If not still in the workspace, let's load the data we saved in the [previous 
reading](../R-03-dplyr), `surveys_complete`.
...
[on the tidyverse site](https://ggplot2.tidyverse.org/reference/ggtheme.html). `theme_minimal()` and `theme_light()` are popular, and `theme_void()` can be useful as a starting
